<template>
  <div class="page-stack">
    ...
    <div class="filter-bar">
      ...
      <select v-model="filters.status" @change="doFetch" class="input">
      ...
      <table v-else class="table-auto">
        ...
        <tbody>
          <tr v-for="p in store.products" :key="p.id">
            ...
            <td>
              <div class="product-cell">
                <div class="product-thumb">
                  <img v-if="p.images?.[0]" :src="imgUrl(p.images[0])" />
            ...
            <td>
              <p class="price-main">{{ fmt(p.price) }}</p>
              <p v-if="p.original_price && p.original_price > p.price" class="price-orig">{{ fmt(p.original_price) }}</p>
            </td>
            <td>
              <span :class="['', p.stock === 0 ? 'stock-out' : p.stock < 10 ? 'stock-low' : 'stock-ok']">{{ p.stock }}</span>
              <p v-if="p.stock === 0" class="stock-hint out">Hết hàng</p>
              <p v-else-if="p.stock < 10" class="stock-hint low">Sắp hết</p>
            </td>
            <td>
              <span :class="statusBadge(p.status)">{{ statusLabel(p.status) }}</span>
              <span v-if="p.is_featured" class="badge" style="background:var(--yellow-100);color:var(--yellow-700);margin-left:4px">⭐</span>
            </td>
            <td>
      ...
      <div v-if="delTarget" class="modal-overlay" @click.self="delTarget = null">
        <div class="modal-box">
          ...
          <div class="modal-body">
            <h3>Xóa sản phẩm?</h3>
            <p>Bạn sắp xóa <strong>{{ delTarget.name }}</strong>.<br>Không thể hoàn tác.</p>
          </div>
...
const hasFilter = computed(() => filters.value.search || filters.value.category_id || filters.value.status)

const BASE   = import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:8000'
const imgUrl = (path: string) => `${BASE}/storage/${path}`
...
function clearFilter() { filters.value = { search: '', category_id: '', status: '' }; doFetch() }

function confirmDel(p: Product) { delTarget.value = p }
