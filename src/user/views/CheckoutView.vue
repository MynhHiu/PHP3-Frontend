            ...
            <div class="form-group full">
              ...
              <div v-else-if="authStore.isLoggedIn && savedAddresses.length > 0" class="addr-select-wrap">
                <div class="addr-select-box">
                  <select class="addr-select" :class="{ 'input-error': errors.address }"
                    ...
                    <option v-for="addr in savedAddresses" :key="addr.id" :value="addr.id">
                      {{ addr.receiver_name }} | {{ addr.phone }}{{ addr.is_default ? ' Mặc định' : '' }} — {{
                ...
                <div v-if="selectedAddressId !== null && selectedAddress" class="addr-preview">
                  ...
                  <div class="addr-preview-text">
                    ...
                    <p>{{ selectedAddress.detail_address }}, {{ selectedAddress.ward }}, {{ selectedAddress.district }},
                      {{ selectedAddress.province }}</p>
                  </div>
                </div>

                <!-- Nhập địa chỉ khác với GHN dropdowns -->
                <div v-if="selectedAddressId === null" class="manual-addr-wrap">
                  <div class="ghn-dropdowns">
                    <div class="ghn-field">
                      <label class="ghn-label">Tỉnh / Thành phố <span class="req">*</span></label>
                      <div class="addr-select-box">
                        <select class="addr-select" :value="manualProvinceId ?? ''"
                          @change="onManualProvinceChange(+($event.target as HTMLSelectElement).value)">
                    ...
                    <div class="ghn-field">
                      <label class="ghn-label">Quận / Huyện <span class="req">*</span></label>
                      <div class="addr-select-box">
                        <select class="addr-select" :disabled="!manualProvinceId" :value="manualDistrictId ?? ''"
                          @change="onManualDistrictChange(+($event.target as HTMLSelectElement).value)">
                          <option value="" disabled>-- Chọn quận/huyện --</option>
                          <option v-for="d in manualDistricts" :key="d.DistrictID" :value="d.DistrictID">{{
                    ...
                    <div class="ghn-field">
                      <label class="ghn-label">Phường / Xã <span class="req">*</span></label>
                      <div class="addr-select-box">
                        <select class="addr-select" :disabled="!manualDistrictId" :value="manualWardCode ?? ''"
                          @change="onManualWardChange(($event.target as HTMLSelectElement).value)">
                          <option value="" disabled>-- Chọn phường/xã --</option>
                          <option v-for="w in manualWards" :key="w.WardCode" :value="w.WardCode">{{ w.WardName }}
                          </option>
                        </select>
                        <span class="select-arrow">▾</span>
                      </div>
                    </div>
                    <div class="ghn-field full">
                      <label class="ghn-label">Số nhà, tên đường <span class="req">*</span></label>
                      <input v-model="manualDetail" type="text" placeholder="VD: 123 Nguyễn Huệ" class="addr-select"
                        style="padding:10px 14px" :class="{ 'input-error': errors.address }"
                        @input="onManualDetailInput" />
                    </div>
                  </div>
                </div>
              ...
              <div v-else>
                ...
                <router-link v-if="authStore.isLoggedIn" to="/profile?tab=address" class="link-add-addr-inline"
                  target="_blank" style="margin-top:8px">
        ...
        <div v-else>
                ...
                <div v-for="c in savedCoupons" :key="(c as any).coupon_code"
                  ...
                  <div class="saved-right">
                    ...
                    <button v-if="(c as any).status === 'active'" class="btn-select-coupon"
                      ...
                      :disabled="couponOk && couponCode !== (c as any).coupon_code" @click="selectSavedCoupon(c)">
          ...
          <div class="summary-lines">
            ...
            <div class="summary-line">
              ...
              <span v-if="loadingFee" class="fee-loading">
                <span class="mini-spinner"></span> Đang tính...
              </span>
              <span v-else :class="shippingFee === 0 ? 'green' : ''">
...
// Lưu district_id và ward_code của địa chỉ đang chọn để tính phí ship và gửi lên backend
const shipDistrictId = ref<number | null>(null)
const shipWardCode = ref<string>('')
const shippingFeeGhn = ref<number>(30_000)  // phí ship thực từ GHN
...
// Hàm tính phí ship, gọi mỗi khi địa chỉ thay đổi
async function recalcShipping() {
  if (!shipDistrictId.value || !shipWardCode.value) {
    shippingFeeGhn.value = 30_000
    return
  }
  loadingFee.value = true
  shippingFeeGhn.value = await ghn.calculateFee(
...
function selectAddress(addr: ShippingAddress) {
  ...
  // ── Lấy GHN ID để tính phí ship ──────────────────────────────────────────
  shipDistrictId.value = addr.ghn_district_id ?? null
  shipWardCode.value = addr.ghn_ward_code ?? ''
  recalcShipping()
}

// ─── Manual address GHN state ─────────────────────────────────────────────────
const manualProvinceId   = ref<number | null>(null)
const manualDistrictId   = ref<number | null>(null)
const manualWardCode     = ref<string>('')
const manualProvinceName = ref('')
const manualDistrictName = ref('')
const manualWardName     = ref('')
const manualDetail       = ref('')
const manualDistricts    = ref<any[]>([])
const manualWards        = ref<any[]>([])

async function onManualProvinceChange(provinceId: number) {
  manualProvinceId.value   = provinceId
  manualDistrictId.value   = null
  manualWardCode.value     = ''
  manualDistrictName.value = ''
  manualWardName.value     = ''
  manualDistricts.value    = []
  manualWards.value        = []
  manualProvinceName.value = ghn.provinces.value.find((p: any) => p.ProvinceID === provinceId)?.ProvinceName ?? ''
  shipDistrictId.value     = null
  shipWardCode.value       = ''
  shippingFeeGhn.value     = 30_000
  buildManualAddress()
  await ghn.fetchDistricts(provinceId)
  manualDistricts.value = ghn.districts.value
}

async function onManualDistrictChange(districtId: number) {
  manualDistrictId.value   = districtId
  manualWardCode.value     = ''
  manualWardName.value     = ''
  manualWards.value        = []
  manualDistrictName.value = manualDistricts.value.find((d: any) => d.DistrictID === districtId)?.DistrictName ?? ''
  shipDistrictId.value     = districtId
  shipWardCode.value       = ''
  shippingFeeGhn.value     = 30_000
  buildManualAddress()
  await ghn.fetchWards(districtId)
  manualWards.value = ghn.wards.value
}

function onManualWardChange(wardCode: string) {
  manualWardCode.value = wardCode
  manualWardName.value = manualWards.value.find((w: any) => w.WardCode === wardCode)?.WardName ?? ''
  shipDistrictId.value = manualDistrictId.value
  shipWardCode.value   = wardCode
  buildManualAddress()
  recalcShipping()
}

function onManualDetailInput() {
  errors.value.address = ''
  buildManualAddress()
}

function buildManualAddress() {
  form.value.address = [manualDetail.value, manualWardName.value, manualDistrictName.value, manualProvinceName.value]
    .filter(Boolean).join(', ')
}
...
function selectManual() {
  ...
  shipWardCode.value       = ''
  shippingFeeGhn.value     = 30_000
  if (!ghn.provinces.value.length) ghn.fetchProvinces()
}
...
const shippingFee = computed(() => {
  if (subtotal.value >= 5_000_000) return 0
  return shippingFeeGhn.value
})
...
async function placeOrder() {
  ...
  try {
    const payload = {
      ...
      // ── GHN: tính phí ship tự động ──────────────────────────────────────────
      to_district_id: shipDistrictId.value ?? undefined,
      to_ward_code: shipWardCode.value || undefined,
      weight: 500,
      // ────────────────────────────────────────────────────────────────────────
